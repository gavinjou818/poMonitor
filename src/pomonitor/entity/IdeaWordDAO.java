 ...
 * IdeaWord entities. Transaction control of the save(), update() and delete()
...
public class IdeaWordDAO implements IIdeaWordDAO 
{
	...
	public void delete(IdeaWord entity) {
		EntityManagerHelper.log("deleting IdeaWord instance", Level.INFO, null);
		try {
			entity = getEntityManager().getReference(IdeaWord.class,
					entity.getId());
	 ...
	 * <pre>
	 * EntityManagerHelper.beginTransaction();
	 * entity = IdeaWordDAO.update(entity);
	 * EntityManagerHelper.commit();
	 * </pre>
	 * 
	 * @param entity
	...
	public IdeaWord update(IdeaWord entity) {
		EntityManagerHelper.log("updating IdeaWord instance", Level.INFO, null);
		try {
			IdeaWord result = getEntityManager().merge(entity);
	...
	public IdeaWord findById(Integer id) {
		EntityManagerHelper.log("finding IdeaWord instance with id: " + id,
				Level.INFO, null);
		try {
			IdeaWord instance = getEntityManager().find(IdeaWord.class, id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<IdeaWord> findByProperty(String propertyName, final Object value) {
		EntityManagerHelper.log("finding IdeaWord instance with property: "
				+ propertyName + ", value: " + value, Level.INFO, null);
		try {
			final String queryString = "select model from IdeaWord model where model."
	...
	public List<IdeaWord> findAll() {
		EntityManagerHelper.log("finding all IdeaWord instances", Level.INFO,
				null);
		try {
			final String queryString = "select model from IdeaWord model";
			Query query = getEntityManager().createQuery(queryString);
			return query.getResultList();
		} catch (RuntimeException re) {

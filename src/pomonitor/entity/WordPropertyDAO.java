 ...
 * WordProperty entities. Transaction control of the save(), update() and
...
public class WordPropertyDAO implements IWordPropertyDAO 
{
	...
	public void save(WordProperty entity) {
		EntityManagerHelper.log("saving WordProperty instance", Level.INFO,
				null);
		try {
	...
	public void delete(WordProperty entity) {
		EntityManagerHelper.log("deleting WordProperty instance", Level.INFO,
				null);
		try {
			entity = getEntityManager().getReference(WordProperty.class,
					entity.getId());
	 ...
	 * <pre>
	 * EntityManagerHelper.beginTransaction();
	 * entity = WordPropertyDAO.update(entity);
	 * EntityManagerHelper.commit();
	 * </pre>
	 * 
	 * @param entity
	...
	public WordProperty update(WordProperty entity) {
		EntityManagerHelper.log("updating WordProperty instance", Level.INFO,
				null);
		try {
			WordProperty result = getEntityManager().merge(entity);
	...
	public WordProperty findById(Integer id) {
		EntityManagerHelper.log("finding WordProperty instance with id: " + id,
				Level.INFO, null);
		try {
			WordProperty instance = getEntityManager().find(WordProperty.class,
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<WordProperty> findByProperty(String propertyName,
			final Object value) {
		EntityManagerHelper.log("finding WordProperty instance with property: "
				+ propertyName + ", value: " + value, Level.INFO, null);
		try {
			final String queryString = "select model from WordProperty model where model."
	...
	public List<WordProperty> findAll() {
		EntityManagerHelper.log("finding all WordProperty instances",
				Level.INFO, null);
		try {
			final String queryString = "select model from WordProperty model";
			Query query = getEntityManager().createQuery(queryString);
			return query.getResultList();
		} catch (RuntimeException re) {
